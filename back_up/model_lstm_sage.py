...
def prepare_graph_data_hourly(data, date, hour, feature_scaler, aqi_scaler, edges):
    ...
    # Lấy đặc trưng không khí và thời gian
...
def predict_aqi_for_future(model, start_data, aqi_scaler, district_names, start_time, future_hours=1):
    ...
    for hour in range(future_hours):
        ...
        hour_predictions = {
            ...
            "predictions": {district_names[i]: round(predictions_original[i][0]) for i in range(len(district_names))}  # Làm tròn AQI đến 2 chữ số thập phân
        }
...
def update_data_for_next_hour(current_data, predicted_aqi):
    ...
    # Cập nhật đặc trưng thời gian (tăng giờ thêm 1)
    new_data.x[:, -2] += 1  # Tăng giá trị của 'hour_of_day'
    new_data.x[:, -2] = new_data.x[:, -2] % 24  # Giới hạn giờ trong ngày (0-23)

    # Nếu giờ chuyển sang 0, tăng ngày trong tuần
    new_data.x[:, -1] = (new_data.x[:, -1] + (new_data.x[:, -2] == 0).float()) % 7  # 'day_of_week'
...
model = GNN_LSTM_Model(num_node_features=10, hidden_size=64, lstm_hidden_size=64, num_layers=2, use_gin=False)
...
# Dự đoán chỉ số AQI cho một ngày giờ cụ thể trong tương lai
district_names = list(data['District'].unique())
start_data = graph_data_hourly[-1]

# Thời gian bắt đầu từ thời điểm hiện tại
start_time = pd.to_datetime(data['dt'].max())  # Sử dụng thời gian cuối cùng từ dữ liệu
...
predictions_df.to_csv('7_days_future_predictions.csv', index=False)
