...
def build_edges_from_distance(data, distance_threshold=50):
    edges = []
...
# Xây dựng cạnh
edges = build_edges_from_distance(data, distance_threshold=30)
...
def prepare_graph_data_hourly(data, date, hour, feature_scaler, aqi_scaler, edges):
    ...
    # Chuẩn hóa các đặc trưng về không khí
...
# Chuẩn bị hai MinMaxScaler riêng biệt cho các đặc trưng không khí và AQI
...
for date in dates:
    print(f"Bắt đầu chuẩn bị dữ liệu cho ngày {date}")
    for hour in hours:
        graph_data = prepare_graph_data_hourly(data, date, hour, feature_scaler, aqi_scaler, edges)
        if graph_data:
            graph_data_hourly.append(graph_data)
            print(f"Chuẩn bị giờ {hour} cho ngày {date}")
            
            # In thông tin chi tiết về đồ thị
            print(f"Số lượng nút (Nodes): {graph_data.num_nodes}")
            print(f"Số lượng cạnh (Edges): {graph_data.num_edges}")
            
            # In chi tiết đặc trưng của các nút và tên huyện (in 5 nút đầu tiên)
            print("Đặc trưng của các nút (Node Features) cho 5 huyện đầu tiên:")
            for i in range(min(5, graph_data.num_nodes)):
                print(f" - Huyện: {district_names[i]}, Features: {graph_data.x[i]}")

            # In danh sách cạnh (in 5 cạnh đầu tiên)
            print("Danh sách các cạnh (Edge Index) cho 5 cạnh đầu tiên:")
            print(graph_data.edge_index[:, :5])

            # In chi tiết nhãn (AQI) cho các nút
            print("Chỉ số AQI (Labels) cho 5 huyện đầu tiên:")
            for i in range(min(5, graph_data.num_nodes)):
                print(f" - Huyện: {district_names[i]}, AQI: {graph_data.y[i].item()}")
            
            print("=" * 50)  # Dấu phân cách giữa các giờ
...
# Hàm dự đoán AQI cho giờ trong tương lai và tính ngày giờ tương ứng
def predict_aqi_for_future(model, start_data, aqi_scaler, district_names, start_time, future_hours=1):
    ...
    # Lặp lại quá trình dự đoán cho từng giờ trong tương lai
    for hour in range(future_hours):
        ...
        hour_predictions = {
            ...
            "predictions": {district_names[i]: predictions_original[i][0] for i in range(len(district_names))}
        }
        ...
        # Chuẩn bị dữ liệu cho giờ tiếp theo
...
model = GNN_LSTM_Model(num_node_features=8, hidden_size=64, lstm_hidden_size=64, num_layers=2, use_gin=True)
...
# Xác định thời gian 0h ngày mai
midnight_tomorrow = (current_time + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)

# Tính toán số giờ còn lại từ thời điểm hiện tại đến 0h ngày mai
time_remaining = midnight_tomorrow - current_time
hours_until_midnight = time_remaining.seconds // 3600
minutes_remaining = (time_remaining.seconds % 3600) // 60

# Dự đoán AQI cho đến 0h ngày mai (bao gồm cả phần giờ lẻ)
district_names = list(data['District'].unique())
start_data = graph_data_hourly[-1]
future_predictions = predict_aqi_for_future(model, start_data, aqi_scaler, district_names, current_time, hours_until_midnight + 1)

# Hiển thị kết quả dự đoán cho từng giờ
...
# Lưu dự đoán vào tệp JSON
prediction_file_path = 'future_predictions.json'
with open(prediction_file_path, 'w') as f:
    json.dump(future_predictions, f, indent=4, default=str)  # default=str giúp ghi cả datetime vào JSON
print(f"Kết quả dự đoán đã được lưu vào {prediction_file_path}")

# Lưu dự đoán vào tệp CSV
predictions_list = []
for prediction in future_predictions:
    for district, aqi in prediction['predictions'].items():
...
predictions_df.to_csv('future_predictions_all_day.csv', index=False)
