...
def predict_aqi_for_future(model, start_data, aqi_scaler, district_names, start_time, future_hours=1):
    ...
    start_day = start_time.date()  # Lưu ngày bắt đầu dự đoán
    ...
    for hour in range(future_hours):
        ...
        # Kiểm tra nếu là ngày đầu tiên thì giữ nguyên dự đoán
        if current_time.date() == start_day:
            adjusted_predictions = predictions_original
        else:
            # Ngày thứ hai trở đi thì điều chỉnh ngẫu nhiên +/- 2 đơn vị
            adjustment = np.random.choice([-1, 1], size=predictions_original.shape)
            adjusted_predictions = predictions_original + adjustment

        # Lưu các dự đoán đã được điều chỉnh hoặc giữ nguyên
        hour_predictions = {
            "timestamp": current_time,
            "predictions": {district_names[i]: max(0, round(adjusted_predictions[i][0])) for i in range(len(district_names))}
        }
        predictions.append(hour_predictions)

        current_time += timedelta(hours=1)
        current_data = update_data_for_next_hour(current_data, adjusted_predictions)

    return predictions

# Khởi tạo và huấn luyện mô hình
model = GAT_LSTM_Model(num_node_features=12, hidden_size=64, lstm_hidden_size=64, num_heads=4, num_layers=2)
